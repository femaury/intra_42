//
//  Constants.swift
//  Intra 42
//
//  Created by Felix Maury on 2019-02-28.
//  Copyright © 2019 Felix Maury. All rights reserved.
//

import UIKit

let IntraTeal = UIColor(hexRGB: "#00BABC")
let ValidGrade = UIColor(hexRGB: "#00CE00")

struct WarningColor {
    static let green = UIColor(hexRGB: "#33BB00")
    static let yellow = UIColor(hexRGB: "#ffcc00")
    static let orange = UIColor(hexRGB: "#ff6700")
    static let red = UIColor(hexRGB: "#cc3300")
}

struct AchievementColor {
    static let none = UIColor(hexRGB: "#E5E5E5")
    static let bronze = UIColor(hexRGB: "#CD7F32")
    static let silver = UIColor(hexRGB: "#C0C0C0")
    static let gold = UIColor(hexRGB: "#E5BF27")
    static let platinum = UIColor(hexRGB: "#14E2D8")
}
